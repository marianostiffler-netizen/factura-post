...
import { comprobanteNro, getSaleMayoristaById, METODO_LABEL } from "@/lib/sales";
...
import { FacturaActions } from "@/app/ventas/[id]/factura-actions";
...
export default async function FacturaMayoristaPage({
  params,
}: {
  params: { id: string };
}) {
  const data = await getSaleMayoristaById(params.id);
  if (!data) notFound();
  ...
  return (
    <main className="mx-auto max-w-3xl p-4 sm:p-6">
      <PageHeader
        title="Factura"
        description={comprobanteNro(sale)}
        backHref="/ventas-mayorista"
      >
        <FacturaActions filename={`factura-mayorista-${comprobanteNro(sale)}`} />
      </PageHeader>
      ...
      >
        ...
        <header className="flex flex-wrap items-start justify-between gap-4 border-b pb-4">
          ...
          <div className="text-right text-sm">
            <span className="inline-block rounded bg-zinc-900 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-white">
              Venta Mayorista
            </span>
            <p className="mt-1 font-semibold">Comprobante</p>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          Venta mayorista · comprobante interno · no válido como factura fiscal (AFIP).
        </p>
      </article>
      ...
      <div className="mt-4 flex justify-center gap-3 print:hidden">
        ...
        <Link
          href="/ventas-mayorista"
          className="text-sm font-medium text-primary hover:underline"
        >
          Ver todas las ventas mayoristas
        </Link>
      </div>
    </main>
  );
